...
//   // Default to "admin" if no search param role is found
//   const [selected, setSelected] = useState<PortalKey>(role ?? "admin");
...
//     e.preventDefault();
//     // Redirects to the dashboard of the currently SELECTED role card
//     navigate({ to: PORTAL_DASHBOARDS[selected] });
//   };

//   // Helper to dynamically show which role the user is filling the form for
//   const getRoleDisplayName = () => {
//     return PORTALS.find((p) => p.key === selected)?.name || "Admin";
//   };
...
//             Sign in to your <span className="font-semibold text-foreground">{getRoleDisplayName()}</span> portal.
//           </p>

//           {/* Form now intercepts the redirection on submit */}
//           <form
...
//             <div className="space-y-2">
//               <Label htmlFor="email">Email</Label>
//               <Input id="email" type="email" placeholder="you@academy.com" defaultValue="demo@neomora.com" required />
//             </div>
//             <div className="space-y-2">
//               <div className="flex items-center justify-between">
//                 <Label htmlFor="password">Password</Label>
//                 <a href="#" className="text-xs text-muted-foreground hover:text-foreground">Forgot password?</a>
//               </div>
//               <Input id="password" type="password" placeholder="password" defaultValue="••••••••" required />
//             </div>
//             <Button type="submit" className="w-full">Sign In as {getRoleDisplayName()}</Button>
//           </form>
...
//               <div className="h-px flex-1 bg-border" />
//             </div>
//             <p className="mb-3 text-xs text-muted-foreground">Select your role to update the form target:</p>
//             <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
//               {PORTALS.map((p) => {
//                 const isSelected = selected === p.key;
//                 return (
...
//                     key={p.key}
//                     type="button"
//                     /* MODIFIED: Clicking only changes state now, it does not navigate */
...
// Superadmin uses a fixed tenant slug. Adjust if yours differs.
const SUPERADMIN_TENANT_SLUG = "neomora-main";
...
function LoginPage() {
  ...
  const getRoleDisplayName = () =>
    PORTALS.find((p) => p.key === selected)?.name ?? "Admin";

  const PORTAL_ALLOWED_ROLES: Record<PortalKey, string[]> = {
    "admin": ["SUPER_ADMIN", "Super Admin"],
    "location-manager": ["LOCATION_MANAGER", "Location Manager"],
    "finance": ["FINANCE_OFFICER", "Finance Officer"],
    "guardian": ["GUARDIAN", "Parent / Guardian", "Guardian", "PARENT", "STAFF", "STAFF_COACH", "Staff/Coach", "Staff", "Coach"],
    "staff": ["STAFF", "STAFF_COACH", "Staff/Coach", "Staff", "Coach"], // ← added "STAFF"
  };
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const allowedRoles = PORTAL_ALLOWED_ROLES[selected];

      if (!allowedRoles.includes(userRole)) {
        authApi.logout();
        throw new Error("Unauthorized Access: Your role does not permit access to this portal.");
      }

      // Navigate to the dashboard for the selected role on success
      navigate({ to: PORTAL_DASHBOARDS[selected] });
    } catch (err) {
    ...
    <div className="flex min-h-screen w-full bg-background">
      ...
      <div className="flex flex-1 items-center justify-center p-6 md:p-10">
        <div className="w-full max-w-md">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Sign in to your{" "}
            <span className="font-semibold text-foreground">{getRoleDisplayName()}</span> portal.
          </p>
